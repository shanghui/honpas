...
contains
  ...
  subroutine setup_hfx_force(libint_data, deriv_data, hfx_optdata, &
    ...
    integer :: BNode, MPIerror, maxnhg, maxnumh, nuog
    integer, pointer :: listhg(:) => null(), listhptrg(:) => null(), &
      numhg(:) => null()
    real(dp), pointer :: Dscfg(:,:) => null()
#endif
    ...
    call re_alloc(Dscfg, 1, maxnhg, 1, hfx_sysdata%nspin, name='Dscfg', &
      routine='setup_hfx_force')
    Dscfg(:,:) = 0.0_dp
      ...
      if ( Node == BNode ) then
        ...
        do ispin=1,hfx_sysdata%nspin
          do jo=1,hfx_sysdata%numh(iio)
            Dscfg(listhptrg(io)+jo,ispin) = &
              Dscf(hfx_sysdata%listhptr(iio)+jo,ispin)
          enddo
        enddo
      endif
      do ispin=1,hfx_sysdata%nspin
        call MPI_Bcast(Dscfg(listhptrg(io)+1,ispin), numhg(io), &
    ...
    call re_alloc(DM_tmp, 1, hfx_sysdata%norb, 1, hfx_sysdata%norb, 1, &
      hfx_sysdata%nspin, name='DM_tmp', routine='setup_hfx_force')
    DM_tmp(:,:,:) = 0.0_dp
    call re_alloc(P_max, 1, hfx_sysdata%norb, 1, hfx_sysdata%norb, &
      name='P_max', routine='setup_hfx_force')
    P_max(:,:) = 0.0_dp

#ifdef MPI
    ! Transform sparse Dm to full matrix
    call sparse_dense(hfx_sysdata%nspin, hfx_sysdata%nuotot, &
      hfx_sysdata%nuotot, hfx_sysdata%norb, maxnhg, numhg, &
      listhptrg, listhg, Dscfg, DM_tmp)
    ! BUG: out-of-bounds, fixed by nuo -> nuotot?
    call get_pmax_shell(hfx_sysdata%nspin, hfx_sysdata%norb, &
      hfx_sysdata%iaorb, hfx_sysdata%iphorb, hfx_sysdata%nuotot, &
      hfx_sysdata%na, hfx_sysdata%isa, maxnhg, numhg, listhptrg, &
      listhg, DM_tmp, P_max)

    call build_hfx_gradient(libint_data, deriv_data, hfx_optdata, &
      hfx_sysdata, maxnhg, numhg, listhptrg, listhg, DM_tmp, P_max, Fal)
#else
    call sparse_dense(hfx_sysdata%nspin, hfx_sysdata%nuotot, &
    ...
    call build_hfx_gradient(libint_data, deriv_data, hfx_optdata, &
      ...
      hfx_sysdata%listhptr, hfx_sysdata%listh, DM_tmp, P_max, Fal)
#endif

    call de_alloc(DM_tmp, name='DM_tmp', routine='setup_hfx_force')
    call de_alloc(P_max, name='P_max', routine='setup_hfx_force')
  ...
  subroutine build_hfx_gradient(libint_data, deriv_data, hfx_opts, hfx_sys, &
               maxnh, numh, listhptr, listh, DM_tmp, P_max, Fal)

    use alloc, only: de_alloc, re_alloc
    use parallel, only: IOnode
    use nao2gto_common
    use nao2gto_data, only: eri_prescreen, hfx_gtos, list_ij, list_kl, &
                            pair_dist_radii_pgf, sfc_pgf, sfc_shell, um_cut
    use nao2gto_libint
    use nao2gto_prescreen
    ...
    do i_list_ij = 1, list_ij%nelement
      ...
      do i_list_kl = 1, list_kl%nelement
        ...
        ks = hfx_sys%isa(hfx_sys%iaorb(ko))
        kspp => species(ks)
        kgto => hfx_gtos(ks)
        ...
        ls = hfx_sys%isa(hfx_sys%iaorb(lo))
        lspp => species(ls)
        lgto => hfx_gtos(ls)
                  ...
                  do nsol = 1, nso(l_l)
                    ...
                    if ( DM_max*maxval(abs( &
                      ...
                      call hfx_gradient_matrix(hfx_sys, num_a, num_b, &
                        num_c, num_d, nao_eri_deriv, DM_tmp, Fal)
                    endif
